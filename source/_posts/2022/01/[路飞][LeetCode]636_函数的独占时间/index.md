...
例如，如果一个函数被调用两次，一次调用执行 `2` 单位时间，另一次调用执行 `1` 单位时间，那么该函数的 **独占时间**`2 + 1 = 3` 。
...
- 我们先将函数的`id`,`类型`,`timestamp`，结构出来
- 如果类型是`start`，则入栈
- 如果类型是`end`出栈，计算耗时
- `end`出栈后，如果栈不为空，说明存在父级函数，需要把父级函数的消耗时间减去当前函数的消耗时间
...
var exclusiveTime = function(n, logs) {
    ...
    for(let i in  logs){
        ...
        if(type=='start'){
            tasks.push({id,type,time})
        }else{
            let cost = time - tasks.pop().time + 1
            res[id] += cost
